...
private:
	...
	void createSwapChain() {
		...
		createInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR; //TODO Test inherit
	...
	void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
		...
		if (vkCreateImage(device, &imageInfo, nullptr, &image) != VK_SUCCESS)
			throw std::runtime_error("Failed to create VK_IMAGE");
	...
	void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
		...
		if (vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
			throw std::runtime_error("Failed to allocate vertyex buffer memory");
	...
	int rateDeviceSuitability(VkPhysicalDevice device) {
		...
		if (!deviceFeatures.geometryShader && !findQueueFamilies(device).isComplete() && checkDeviceExtensionSupport(device) && !querySwapChainSupport(device).isCapable())
			score = 0;
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(30.0f), glm::vec3(-1.0f, 1.0f, 1.0f));
