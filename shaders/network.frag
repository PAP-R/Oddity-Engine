...
float function(uint func, float value) {
    switch(func) {
        case 0:
            return -x;
        case 1:
...
float[MAX_THROUGHPUT] apply(uint net_start, float[MAX_THROUGHPUT] net_input) {
    ...
    uint layer_couht = nets[net_start + 1];

    uint layer_start = net_start + 2;

    for (uint l = 0; l < layer_count; l++) {
        uint weight_start = layer_start + 2;
        uint bias_start = weight_start + nets[weight_start];
        uint function_start = bias_start + nets[bias_start];
        ...
        for (uint x = 0; x < nets[weight_start + 1]; x++) {
            ...
            weight_layer_start = weight_layer_start + nets[weight_layer_start];
        }
        ...
        for (uint x = 0; x < nets[function_start + 1]; x++) {
            for (uint y = 0; y < nets[function_layer_start + 1]; y++) {
                throughput[x] = function(nets[function_layer_start + 2 + y], net_input[y]);
            }
            function_layer_start = function_layer_start + nets[function_layer_start];
        }

        net_input = throughput;

        layer_start = layer_start + nets[layer_start];
    }
