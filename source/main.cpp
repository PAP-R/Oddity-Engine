...
int main() {
    ...
    auto glow_white = OddityEngine::Graphics::Buffer::Bufferobject(&tracer->materialbuffer, OddityEngine::Graphics::buffermaterial(vec4(1), vec4(0, 1, 1, 1), 1));
    auto glow_grey = OddityEngine::Graphics::Buffer::Bufferobject(&tracer->materialbuffer, OddityEngine::Graphics::buffermaterial(vec4(0.5), vec4(1, 0, 0, 1), 1));

    auto cube = OddityEngine::Graphics::Buffer::Bufferobject(&tracer->objectbuffer, OddityEngine::Graphics::bufferobject(OddityEngine::Graphics::transform(vec3(0, 0, 0.7), vec3(0), vec3(0.2)), 1, OddityEngine::Graphics::MESH, glow_white.get_index(), cube_vertices[0].get_index(), cube_vertices.size()));

//    auto orb = OddityEngine::Graphics::Buffer::Bufferobject(&tracer->objectbuffer, OddityEngine::Graphics::bufferobject(OddityEngine::Graphics::transform(vec3(0, 0, 0.7), vec3(0), vec3(2)), 0.2, OddityEngine::Graphics::SPHERE, basic_red_transparent.get_index()));
...
//        inner_orb.set(OddityEngine::Graphics::bufferobject(OddityEngine::Graphics::transform(vec3(sin(time) * 0.2, cos(time) * 0.2, 0.7 + sin(time) * 0.2), vec3(0), vec3(0.5)), 0.2, OddityEngine::Graphics::SPHERE, basic_blue.get_index()));
        cube.set(OddityEngine::Graphics::bufferobject(OddityEngine::Graphics::transform(vec3(0, 0, 0.7), vec3(sin(time), cos(time), sin(time)), vec3(0.2)), 1, OddityEngine::Graphics::MESH, glow_white.get_index(), cube_vertices[0].get_index(), cube_vertices.size()));
