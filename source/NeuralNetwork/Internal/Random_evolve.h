...
namespace OddityEngine::NeuralNetwork {
    ...
    protected:
        inline static Vector<std::function<float(float, float)>> available_functions = {
            ...
            [](const float x, const float y){return sqrt(x);},
            [](const float x, const float y){return x * x;},
            [](const float x, const float y){return x * x * x;},
            ...
            [](const float x, const float y){return 1 / (1 + pow(std::numbers::e, x));},
    ...
    public:
        Random_evolve(const size_t input_count = 1, const size_t output_count = 1, float weight_chance = 0.4, float bias_chance = 0.4, float function_chance = 0.2) : Layer(input_count, output_count), weight_chance(weight_chance), bias_chance(bias_chance), function_chance(function_chance) {
        ...
        Random_evolve(const Matrix<>& weights, const Vector<>& bias, const Vector<size_t>& functions, float weight_chance = 0.4, float bias_chance = 0.4, float function_chance = 0.2) : Layer(weights, bias, functions), weight_chance(weight_chance), bias_chance(bias_chance), function_chance(function_chance) {
            float total_chance = weight_chance + bias_chance + function_chance;
            this->weight_chance /= total_chance;
            this->bias_chance /= total_chance;
            this->function_chance /= total_chance;
        ...
        Random_evolve(const Vector<float>& csv, float weight_chance = 0.4, float bias_chance = 0.4, float function_chance = 0.2) : Layer(csv[3], csv[5]), weight_chance(weight_chance), bias_chance(bias_chance), function_chance(function_chance) {
            float total_chance = weight_chance + bias_chance + function_chance;
        ...
        void evolve(float rate) override {
            ...
            if (random < weight_chance) {
                ...
                this->functions.push_back(Math::random<size_t>(1, available_functions.size() - 1));
                this->functions.push_back(Math::random<size_t>(0, output_size() - 1));
                this->functions.push_back(Math::random<size_t>(0, output_size() - 1));
            }
        }
        ...
        Vector<> forward(const Vector<>& input) override {
            ...
            for (size_t i = 0; i < functions.size(); i += 3) {
                auto x = i + 1;
                auto y = i + 2;
                output[x] = available_functions[i](output[x], output[y]);
            }
            return output;
        }
        ...
        static Random_evolve from_csv(const Vector<>& csv) {
            ...
            auto bias = wbf[1];
            auto functions_csv = wbf[2];
            Vector<size_t> functions(functions_csv.size());

            for (size_t i = 0; i < functions_csv.size(); i++) {
                functions[i] = functions_csv[i];
            }
